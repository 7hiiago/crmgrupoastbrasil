...
function CRMApp() {
  ...
  const [currentNegId, setCurrentNegId] = useState<string | undefined>(undefined);
  ...
  // Navegação com suporte a negociacaoId
  const navigateTo = (p: string, negId?: string) => {
    setCurrentNegId(negId);
    setPage(p);
  };
  ...
  const renderPage = () => {
    switch (page) {
      ...
      case 'negociacoes': return <NegociacoesPage onNavigate={navigateTo} />;
      case 'proposta':    return <PropostaPage negociacaoId={currentNegId} onBack={() => setPage('negociacoes')} />;
