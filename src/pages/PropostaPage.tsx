...
const salvar = () => {
  ...
  try {
    ...
    // Atualiza o valor da negociação com o total das seções
    if (negociacaoId) {
      ...
      {aba === 'preview' && (
        <div className="space-y-4">
          <p className="text-xs text-ast-text3">
            Preview do documento. Use "Baixar PDF" para exportar.
          </p>
                      ...
                      <SlideLayout corPrimaria={cfg.corPrimaria} logoUrl={cfg.logoUrl} nomeEmpresa={cfg.nomeEmpresa}>
                        <div style={{ padding: '32px 48px' }}>
                          <div style={{ marginBottom: '16px' }}>
                            <div style={{ display: 'inline-block', borderLeft: `4px solid ${cfg.corPrimaria}`, paddingLeft: '12px' }}>
                            ...
                            <tbody>
                              {sec.itens.map((item, idx) => (
                                <tr key={item.id} style={{ background: idx % 2 === 0 ? '#f9f9f9' : '#fff' }}>
                                  ...
                                  <td style={{ padding: '6px 8px', textAlign: 'right', color: '#555' }}>{fmt(item.valorUnd)}</td>
                                  <td style={{ padding: '6px 8px', textAlign: 'right', color: '#333', fontWeight: 500 }}>{fmt(item.qtd * item.valorUnd)}</td>
                                </tr>
                              ))}
                            </tbody>
                            <tfoot>
                              <tr style={{ background: '#eee' }}>
                                <td colSpan={3} style={{ padding: '7px 12px', fontWeight: 700, fontSize: '12px' }}>TOTAL PRODUTO</td>
                                <td style={{ padding: '7px 8px', textAlign: 'right', fontWeight: 700, fontSize: '12px' }}>{fmt(tot.produto)}</td>
                              </tr>
                              {sec.servicoValor > 0 && (
                                <tr style={{ background: '#d6f0d6' }}>
                                  <td colSpan={3} style={{ padding: '7px 12px', fontWeight: 700, fontSize: '12px', color: '#2a6e2a' }}>SERVIÇO</td>
                                  <td style={{ padding: '7px 8px', textAlign: 'right', fontWeight: 700, fontSize: '12px', color: '#2a6e2a' }}>{fmt(sec.servicoValor)}</td>
                                </tr>
                              )}
                            </tfoot>
                          </table>
                ...
                <PaginaPDF orientacao={cfg.orientacao} className="quebra-pagina">
                  <SlideLayout corPrimaria={cfg.corPrimaria} logoUrl={cfg.logoUrl} nomeEmpresa={cfg.nomeEmpresa}>
                    <div style={{ padding: '40px 60px' }}>
                      ...
                      {cfg.secoes.map(s => {
                        ...
                        return (
                          <div key={s.id} style={{ marginBottom: '16px' }}>
                            <div style={{ color: cfg.corPrimaria, fontWeight: 700, fontSize: '12px', textTransform: 'uppercase' }}>{s.titulo}</div>
                            <div style={{ color: '#333', fontSize: '12px' }}>PRODUTO: {fmt(t.produto)} &nbsp;&nbsp; SERVIÇO: {fmt(t.servico)}</div>
                          </div>
                        );
                      })}
                      <div style={{ marginTop: '24px', padding: '16px 20px', background: '#111', borderRadius: '4px', display: 'flex', gap: '40px' }}>
                        <div>
                      ...
                      {cfg.observacoes && (
                        <p style={{ color: '#777', fontSize: '10px', marginTop: '20px', lineHeight: 1.6 }}>{cfg.observacoes}</p>
                      )}
                    </div>
                  </SlideLayout>
                </PaginaPDF>
