...
export default function NegociacoesPage({ onNavigate }: { onNavigate?: (page: string) => void }) {
		...
		<div className="flex gap-1" onClick={e => e.stopPropagation()}>
 		   ...
 		   <Btn variant="icon" sm onClick={() => enviarPropostaWhatsApp(n)}>💬</Btn>
  		   <Btn variant="icon" sm onClick={() => showHistory(n.id)}>📋</Btn>
  		      {onNavigate && (
   		   <Btn variant="icon" sm onClick={() => onNavigate('proposta')} className="text-[11px]" title="Gerar proposta visual">
		      🖊
 			   </Btn>
				  )}
